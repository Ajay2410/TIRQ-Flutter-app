...
class SupplierMachineDetailsView extends StatefulWidget {
  ...
  const SupplierMachineDetailsView({super.key, required this.machineElement, required this.organizationId});

  @override
  State<SupplierMachineDetailsView> createState() => _SupplierMachineDetailsViewState();
}

class _SupplierMachineDetailsViewState extends State<SupplierMachineDetailsView> with SingleTickerProviderStateMixin {
  ...
  Future<void> _onOnlineSupportPressed(SupplierMachineDetailsViewModel viewModel) async {
    _toggle();
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<SupplierMachineDetailsViewModel>.reactive(
      viewModelBuilder: () => SupplierMachineDetailsViewModel()..init(widget.machineElement.machine?.id ?? '', widget.organizationId),
                  ...
                  child: Container(
                    ...
                    child:
                        viewModel.isBusy
                            ...
                            : SingleChildScrollView(padding: const EdgeInsets.all(16), child: _buildMachineDetails()),
                  ),
                ),
              ],
            ),
  ...
  String getMachineType() => widget.machineElement.machine?.machineType ?? 'Unknown Type';

  String getModelNumber() => widget.machineElement.machine?.modelNumber ?? 'N/A';

  String getTotalPower() => widget.machineElement.machine?.totalPower?.toString() ?? '0';
  ...
  String getMaxHeight() => widget.machineElement.machine?.processingDimensions?.maxHeight?.toString() ?? 'N/A';

  String getMaxWidth() => widget.machineElement.machine?.processingDimensions?.maxWidth?.toString() ?? 'N/A';

  String getMinHeight() => widget.machineElement.machine?.processingDimensions?.minHeight?.toString() ?? 'N/A';

  String getMinWidth() => widget.machineElement.machine?.processingDimensions?.minWidth?.toString() ?? 'N/A';
  ...
  WarrantyStatus? get warrantyStatusEnum => WarrantyStatus.fromString(widget.machineElement.warrantyStatus);
  ...
  Widget _buildExpandableFloatingActionButton(SupplierMachineDetailsViewModel viewModel) {
  ...
  List<Widget> _buildExpandingActionButtons(SupplierMachineDetailsViewModel viewModel) {
