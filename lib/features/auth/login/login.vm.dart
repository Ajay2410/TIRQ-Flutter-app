...
class LoginViewModel extends ReactiveViewModel {
  ...
  LoginMode _forgotPasswordMode = LoginMode.email;
  ...
  Future<void> verifyOtp() async {
    ...
    try {
      ...
      final response = await authService.otpLogin(email: contact, otp: otpController.text);

      response.fold(
        (failure) {
          Fluttertoast.showToast(msg: failure.message);
          _clearOtpDigits(); // Clear OTP on failure
        },
        (user) async {
          await saveUser(user);
          navigateToStageView();
  ...
  Future<void> sendPasswordResetOtp() async {
    ...
    try {
      ...
      final response = await authService.sendPasswordResetOtp(email: contact);
  ...
  Future<void> verifyPasswordResetOtp() async {
    if (forgotOtpController.text.length != 6) {
      Fluttertoast.showToast(msg: 'Please enter complete OTP');
      return;
    }
    ...
    try {
      ...
      final response = await authService.verifyPasswordResetOtp(email: contact, otp: forgotOtpController.text.trim());

      response.fold(
        (failure) {
          Fluttertoast.showToast(msg: failure.message);
          _clearForgotOtpDigits(); // Clear OTP on failure
        },
        (success) {
          _forgotPasswordStep = ForgotPasswordStep.newPassword;
      ...
      AppLogger.error('OTP verification error: $e');
      Fluttertoast.showToast(msg: 'Invalid OTP. Please try again.');
      _clearForgotOtpDigits(); // Clear OTP on error
    } finally {
      _isBusyForgotPassword = false;
      notifyListeners();
    }
  ...
  Future<void> resetPassword() async {
    ...
    try {
      ...
      final response = await authService.resetPassword(
        email: contact,
        otp: getForgotPasswordOtp().trim(), // Use forgot password OTP
  ...
  Future<void> sendOtpForLogin() async {
    if (forgotPasswordFormKey.currentState?.validate() != true) {
      return;
    }
  ...
  ResultFuture<User> facebookLogin() async {
    return await authService.facebookLogin(
      ...
      token: passwordController.text, // You might want to use Facebook access token here
    );
  }
