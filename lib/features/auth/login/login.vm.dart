  ...
  class LoginViewModel extends ReactiveViewModel {
    ...
    Future<void> verifyOtp() async {
      ...
      try {
        ...
        String contact = _forgotPasswordMode == LoginMode.email
            ? forgotEmailController.text.trim()
            : _forgotFullPhoneNumber;
        final response = await authService.otpLogin(
          email: contact,
    ...
    Future<User?> _convertFacebookUserToAppUser(
        firebase_auth.User? firebaseUser, Map<String, dynamic> facebookData) async {
      if (firebaseUser == null) return null;
    ...
    Future<void> _handleOtpSend() async {
      ...
      try {
        String contact = _loginMode == LoginMode.email
